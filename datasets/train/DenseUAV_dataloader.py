...
from datasets.train.DenseUAV_dataset import DenseUAVDataset
from datasets.dataset_transforms.DenseUAV_transform import get_DenseUAV_transforms
...
class DenseUAVDataModule(pl.LightningModule):
    def __init__(self,
                 batch_size,
                 image_size=(480, 640),
                 sample_num=4,
        ...
        self.drone_transform, self.sat_transform, _ = (
            get_DenseUAV_transforms(img_size=self.image_size,padding=0,**data_augmentation
                                    ))
    ...
    def reload(self):
        self.train_dataset = DenseUAVDataset(sat_transform=self.sat_transform,
...
if __name__ == '__main__':
    ...
    datasets = DenseUAVDataset(sat_transform=transform_train_list['satellite'],drone_transform=transform_train_list['train'],
                            sources=['satellite', 'drone'])
    samper = Sampler_University(datasets,8)
    dataloader = DataLoader(datasets,batch_size=8,num_workers=0,sampler=samper,collate_fn=train_collate_fn,drop_last=True)
    print(1)
