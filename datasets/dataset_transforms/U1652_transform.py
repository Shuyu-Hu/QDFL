...
def get_U1652_transforms(img_size=(256,256),padding=0):
    (h,w) = img_size
    # transform_train_drone_list = transforms.Compose([
    ...
    #     ])
    #
    # transform_train_satellite_list = transforms.Compose([
    #     transforms.Resize((h, w), interpolation=transforms.InterpolationMode.BILINEAR),
    #     transforms.Pad(padding, padding_mode='edge'),
    ...
    #     transforms.Resize(size=(h, w), interpolation=transforms.InterpolationMode.BILINEAR),  # Image.BICUBIC
    #     transforms.ToTensor(),
    #     transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    # ])

    """
    以下是sample4geo的一些改动，尝试一下是否对性能有提升
    """
    ...
    # transform_train_drone_list = A.Compose([A.ImageCompression(quality_lower=80, quality_upper=100, p=0.5),
    #                                     A.Resize(h, w, interpolation=cv2.INTER_LINEAR_EXACT, p=1.0),
    #                                     A.ColorJitter(brightness=0.3, contrast=0.35, saturation=0.25, hue=0.25, always_apply=False, p=0.5),
    #                                     A.OneOf([
    #                                              A.AdvancedBlur(p=1.0),
    #                                              A.Sharpen(p=1.0),
    #                                           ], p=0.3),
    ...
    #                                              A.CoarseDropout(max_holes=25,
    #                                                              max_height=int(0.2*h),
    #                                                              max_width=int(0.2*h),
    #                                                              min_holes=10,
    #                                                              min_height=int(0.1*h),
    #                                                              min_width=int(0.1*h),
    #                                                              p=1.0),
    #                                           ], p=0.3),
    #                                     A.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]),
    ...
    #                                     ])
    #
    # transform_train_satellite_list = A.Compose([A.ImageCompression(quality_lower=80, quality_upper=100, p=0.5),
    #                                       A.Resize(h, w, interpolation=cv2.INTER_LINEAR_EXACT,
    #                                                p=1.0),
    #                                       A.ColorJitter(brightness=0.3, contrast=0.35, saturation=0.25, hue=0.25,
    #                                                     always_apply=False, p=0.5),
    #                                       A.OneOf([
    #                                           A.AdvancedBlur(p=1.0),
    #                                           A.Sharpen(p=1.0),
