...
class DenseUAV_model(pl.LightningModule):
    def __init__(self,
                 backbone_name,
                 backbone_configs,
                 components_name,
    ...
    def on_train_epoch_start(self) -> None:
        self.preds, self.preds2, self.labels_sat, self.labels_drone = [], [], [], []

    def training_step(self, batch, batch_idx):
        ((inputs_sat, labels_sat),
        ...
        contrast = False
        if out_sat[1] and out_drone[1]:
            contrast = True
        #out_sat/out_drone[1] indicates the output feature/descriptor
        # contrast_loss = self.InfoNCEloss(out_sat[1], out_drone[1], labels_sat)
        if contrast:
            if 'triplet' in self.which_contrast_loss.lower():
        ...
        cls_loss = Loss.cal_loss(out_sat, labels_sat, self.CEloss) + Loss.cal_loss(out_drone, labels_drone,
                                                                   self.CEloss)  # only compute the global branch
        if self.add_KLloss:
            ...
            # klloss = Loss.cal_kl_loss(out_drone, out_sat, self.KLloss)
    ...
    def calculate_accuracy(self,preds,labels):
        ...
        if not isinstance(preds, list):
            preds = [preds]
        '''
        Since the model has multiple cls head, so we have to calculate each cls head's accuracy, 
        and then sum it up. 
        '''
