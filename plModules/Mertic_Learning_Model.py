...
class Metric_Learning_model(pl.LightningModule):
    def __init__(self,
                 backbone_name,
                 backbone_configs,
                 components_name,
        ...
        self.backbone_lr_scalar = backbone_lr_scalar
        ...
        if 'triplet' in self.which_contrast_loss.lower():
            ...
            self.MSloss = losses.MultiSimilarityLoss(alpha=2.0, beta=50, base=0.1, distance=DotProductSimilarity())
            ...
            self.InfoNCEloss = loss_func.InfoNCE_loss(self.CEloss, self.device)
            self.tau = nn.Parameter(torch.ones([1]) * np.log(1 / 0.07))
        self.preds, self.preds2, self.preds3, self.labels_query, self.labels_street, self.labels_database = [], [], [], [], [], []
    ...
    def configure_optimizers(self):
        ...
        if self.optimizer.lower() == 'sgd':
            base_optimizer = torch.optim.SGD([
                {'params': base_params, 'lr': self.backbone_lr_scalar * self.lr},
    ...
    def on_train_epoch_start(self) -> None:
        (self.preds, self.preds2, self.labels_query,
         self.labels_street, self.labels_database) = [], [], [], [], []

    def training_step(self, batch, batch_idx):
        ((inputs_query, labels_query), (inputs_database, labels_database)) = batch
        self.labels_query.append(labels_query)
        self.labels_database.append(labels_database)
        out_query = self(inputs_query)
        out_database = self(inputs_database)
        # out_query/out_database[1] indicates the output feature/descriptor
        if 'triplet' in self.which_contrast_loss.lower():
            contrast_loss = Loss.cal_triplet_loss(out_query, out_database, labels_query, self.Tripletloss)
            contrast_loss += Loss.cal_triplet_loss(out_database, out_query, labels_query, self.Tripletloss)
        elif 'multi' in self.which_contrast_loss.lower():
            contrast_loss = Loss.cal_MS_loss(out_query, out_database, labels_query, self.MSloss, self.MSminer)
            contrast_loss += Loss.cal_MS_loss(out_database, out_query, labels_query, self.MSloss, self.MSminer)
        elif 'infonce' in self.which_contrast_loss.lower():
            contrast_loss = self.InfoNCEloss(out_query, out_database, self.tau)
            contrast_loss += self.InfoNCEloss(out_database, out_query, self.tau)

        if self.add_MMDloss:
            # In order to realise domain generalization, extract the fusion_feature to calculate mmd loss
            mmdloss = 0.5*cal_mmd_loss(out_database[2], out_query[2], self.MMDloss) + 0.5*cal_mmd_loss(out_query[2], out_database[2], self.MMDloss)
        else:
            mmdloss = torch.tensor((0))

        if self.add_MSEloss:
            # In order to realise domain generalization, extract the fusion_feature to calculate mmd loss
            mseloss = cal_mse_loss(out_database[2], out_query[2], self.MSEloss)
        else:
            mseloss = torch.tensor((0))

        if self.add_KLloss:
            # input: database view, target: queryellite view
            ...
            # klloss = 0.5*Loss.cal_kl_loss(out_database, out_query, self.KLloss)
            # klloss += 0.5*Loss.cal_kl_loss(out_query, out_database, self.KLloss)
            klloss = 0.5*Loss.cal_js_loss(out_database, out_query, self.KLloss)
            klloss += 0.5*Loss.cal_js_loss(out_query, out_database, self.KLloss)
        else:
        ...
        if self.grad_optimizer_name:
            ...
            loss = contrast_loss + klloss + 0.2*mmdloss + mseloss
...
def main():
    ...
    m = Metric_Learning_model(**options)
    z = m(x)
