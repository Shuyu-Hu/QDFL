...
# from layers import Mlp, PatchEmbed, SwiGLUFFNFused, MemEffAttention, NestedTensorBlock as Block  #test
...
class DinoVisionTransformer(nn.Module):
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        if self.interpolate_offset:
            ...
            kwargs["scale_factor"] = sx #test GFLOPs
        else:
...
def throughput(model, img_size=224, bs=1):
    with torch.no_grad():
        ...
        # model_dino=create_model('vit_base_patch16_224_in21k', checkpoint_path='./ViT-B_16.npz', drop_path_rate=0.1)
        model.eval()
        for i in range(500):
            # with amp_autocast():
            model(x)
