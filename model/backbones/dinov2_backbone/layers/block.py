...
#     def __init__(self, D_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., skip_connect=True, mlp_ratio=None):
#         super().__init__()
#         self.skip_connect = skip_connect
#         out_features = out_features or D_features
#         hidden_features = hidden_features or D_features
#         hidden_features = int(hidden_features / 3)
#         self.fc1 = nn.Linear(D_features, hidden_features * 2)
...
# class Adapter(nn.Module):
#     def __init__(self, adapter_dim, hidden_dim=384, dropout=0.2, skip_connect=True):
...
class Block(nn.Module):
    def __init__(
        ...
        # self.att_adapter = ARC_adapter(adapter_dim=dim,hidden_dim=dim,dropout=0.0,position='att')
        # self.mlp_adapter = ARC_adapter(adapter_dim=dim,hidden_dim=dim,dropout=0.0,position='mlp')
        # self.att_proj = nn.Parameter(torch.empty(dim, dim))
        # self.mlp_proj = nn.Parameter(torch.empty(dim, dim))
        # nn.init.xavier_uniform_(self.mlp_proj)
        # nn.init.xavier_uniform_(self.att_proj)

    def forward(self, x: Tensor) -> Tensor:
        def attn_residual_func(x: Tensor) -> Tensor:
            return self.ls1(self.adapter1(self.attn(self.norm1(x))))
            # if self.att_adapter is not None and self.att_proj is not None:
            #     return self.ls1(self.att_adapter(self.attn(self.norm1(x)),self.att_proj,self.att_proj.t()))
            # return self.ls1(self.attn(self.norm1(x)))

        def ffn_residual_func(x: Tensor) -> Tensor:
            return self.ls2(self.mlp(self.norm2(x))+0.2*self.adapter2(self.norm2(x)))
            # if self.mlp_adapter is not None and self.mlp_proj is not None:
            #     return self.ls2(self.mlp_adapter(self.norm2(x),self.mlp_proj,self.mlp_proj.t()))
    ...
    def forward_nested(self, x_list: List[Tensor]) -> List[Tensor]:
        ...
        if self.training and self.sample_drop_ratio > 0.0:
            ...
            def attn_residual_func(x: Tensor, attn_bias=None) -> Tensor:
                return self.ls1(self.adapter1(self.attn(self.norm1(x))))
                # if self.att_adapter is not None and self.att_proj is not None:
                #     return self.att_adapter(self.attn(self.norm1(x)), self.att_proj, self.att_proj.t())
                # return self.attn(self.norm1(x), attn_bias=attn_bias)

            def ffn_residual_func(x: Tensor, attn_bias=None) -> Tensor:
                return self.ls2(self.mlp(self.norm2(x))+0.2*self.adapter2(self.norm2(x)))
                # if self.mlp_adapter is not None and self.mlp_proj is not None:
                #     return self.mlp_adapter(self.norm2(x), self.mlp_proj, self.mlp_proj.t())
