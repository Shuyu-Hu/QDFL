...
class Tripletloss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        """
        #输入的inputs的形状是[2*B,bottleneck_dim]
        n = inputs.size(0)
        ...
        #mask：生成一个掩码矩阵，用于指示哪些样本属于同一类。mask[i][j] 为 True 表示样本 i 和样本 j 属于同一类。
        mask = targets.expand(n, n).eq(targets.expand(n, n).t())#自己和自己的转置阵进行mask运算，一样为真，不一样为假
        dist_ap, dist_an = [], []
        for i in range(n):
            if i < n/2:
                dist_ap.append(dist[i][int(n/2):n][mask[i][int(n/2):n]].max().unsqueeze(0))#正样本为距离最大的
                dist_an.append(dist[i][int(n/2):n][(mask[i] == 0)[int(n/2):n]].min().unsqueeze(0))#负样本为距离最小的
            else:
